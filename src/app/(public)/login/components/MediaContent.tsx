...
export const MediaContent: React.FC<MediaContentProps> = ({
  ...
  // Reset image state when switching to image mode
  useEffect(() => {
    if (mediaMode === "image") {
      setImageError(false);
  ...
  if (mediaMode === "image") {
    return (
      <div className="relative w-full h-full">
        ...
        {/* Show fallback content if image failed */}
        {imageError && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center text-white/80">
              ...
              <p className="text-lg font-light">Institución Universitaria del Putumayo</p>
            </div>
          </div>
        )}
      </div>
