...
interface CategoriaAspectoViewProps {
  ...
  onVerAspectos: (categoria: CategoriaAspecto) => void;
  pagination?: PaginationMeta | null;
  onPageChange: (page: number) => void;
  onLimitChange: (limit: number) => void;
}

export function CategoriaAspectoView({
  categorias,
  ...
  onPageChange,
  onLimitChange,
}: CategoriaAspectoViewProps) {
  return (
    <Card>
      <CardHeader>
        <div className="flex justify-between items-start">
          <div>
            <CardTitle>Gestión de Categorías de Aspectos de Evaluación</CardTitle>
            <CardDescription>
              Organiza los aspectos de evaluación en categorías temáticas como "Pedagógico", "Investigación", "Administrativo", etc.
            </CardDescription>
          </div>
          <Button onClick={() => setModalCategoriaAspecto({ isOpen: true, categoria: undefined })}>
            ...
            <Card className="border-dashed border-2 border-muted">
              <CardContent className="p-8 text-center">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Crea tu primera categoría para organizar los aspectos de evaluación
                </p>
              </CardContent>
            </Card>
          )}
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                  ...
                  <div className="flex gap-2 self-start items-center">
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => onVerAspectos(categoria)}
                      title="Ver aspectos asociados"
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => handleEliminarCategoriaAspecto(categoria)}
                      title="Eliminar"
                      className="hover:bg-muted hover:text-primary"
                    >
